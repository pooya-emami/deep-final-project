...
Transformer models have become the dominant architecture in modern deep learning. Since the introduction of the original Transformer \cite{vaswani2017attention}, this architecture has been scaled to create large language models with billions of parameters. Models like GPT, LLaMA, and OPT have demonstrated strong performance across various natural language tasks, but their large size makes deployment difficult on devices with limited memory.

Quantization has been shown to be effective for reducing model size \cite{gptq,lin2024awq}. By reducing the precision of weights from 16-bit floats to 4-bit integers, the memory footprint can be reduced by roughly 75\%. Post-Training Quantization (PTQ) is particularly attractive because it does not require retraining the model, using only a small calibration dataset to determine quantization parameters.
...
SmoothQuant \cite{xiao2023smoothquant} takes a closely related view, migrating quantization difficulty from activations to weights via a similar per-channel scaling, enabling joint weight-activation quantization. Outlier Suppression+ \cite{wei2023outlier} further extends this with channel-wise shifting.
...
\end{equation}
where $C(b_i)=b_i$ is the per-block memory cost and $B$ is the total budget, guaranteeing exactly one bit-width per block under the memory constraint. As discussed in Sec.~4.6, this additive objective assumes blocks contribute independently to total model degradation, which is an approximation we did not validate against the true joint (all-blocks-quantized-simultaneously) loss.
...
We evaluate on GPT-2 (124M parameters, FP32) and Llama-3.2-1B (1.2B parameters, native bfloat16). Calibration uses 256 samples from Open-Platypus \cite{openplatypus} or WikiText-2 \cite{wikitext}, tokenized into 128-token chunks and evaluated on a disjoint held-out set of 256 further samples. All results use seed 42; we did not average over multiple seeds, which we note as a limitation. We use HuggingFace Transformers \cite{huggingface} and \texttt{llmcompressor} \cite{llmcompressor} for baselines. Our method uses 200 calibration steps per layer, batch size 8, and 1--3 passes.

Perplexity (PPL), the standard language-modeling metric \cite{wikitext}, is computed as:
\begin{equation}
...
Table~\ref{tab:mixed} shows MCKP allocation on GPT-2/Open-Platypus. At 4-bit average, PPL is 24.61; at 3.5-bit it rises to 26.99; at 5-bit it drops to 24.24 -- within 0.15 of FP32.
...
Several limitations should be acknowledged. The sensitivity metric remains a heuristic; we have not compared it against Fisher information or Hessian trace. The MCKP formulation assumes independent block contributions, ignoring error compounding through the residual stream. This limitation is shared with MixFrag. We also did not ablate the KL term or $W_O$ inclusion, and report only perplexity, leaving latency and memory measurements for future work.
