...
def build_awq_recipe(arch, include_wo=True, num_bits=4):
    if arch == "gpt2":
        ...
        mappings = [{
            ...
            "balance_layers": [r"re:transformer\.h\.\d+\.attn\.c_attn$"],
        }]
    ...
    return [
        AWQModifier(mappings=mappings, duo_scaling=True),
...
def run_awq_quantization(MODEL_ID, calib_corpus, eval_corpus, output_dir, include_wo=True, num_bits=4, seed=42):
    ...
    recipe = build_awq_recipe(arch, include_wo=include_wo, num_bits=num_bits)
